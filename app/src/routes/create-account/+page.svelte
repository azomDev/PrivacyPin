<script lang="ts">
	...
	async function connectCheck() {
		if (server_url != "" && signup_key != "") {
			showModal(
				"Beta Notification",
				"This would have run signup checks with the data:" + "{'" + server_url + "', '" + signup_key + "'}\n\nYou will now be redirected to the home page",
			...
			showModal("Connection Error", "Please enter a valid Server Address and Signup Key");
		}
	}
</script>
...
<div class="inner-card">
	<div class="dot" style="background-color: #d298eb;">
		<MapPin color="#a538d1" size={50} style="margin-top: 25; margin-bottom: 25;" />
	</div>
	<h1 style="line-height: .5">Privacy Pin</h1>
	<p style="line-height: 1">Connect with your sever to start sharing</p>
	<br />

	<h4 style="text-align: left; line-height: 0.1; margin: auto; padding: 16px 32px;">Server Address</h4>
	<input class="button-secondary" style="width: 75%; text-align: left;" placeholder="http://your-server.com" bind:value={server_url} required />
	<br /><br />
	<h4 style="text-align: left; line-height: 0.1; margin: auto; padding: 16px 32px;">Signup Key</h4>
	<input class="button-secondary" style="width: 75%; text-align: left;" placeholder="Enter your signup key" bind:value={signup_key} required />
	<p style="text-align: left; color: a9a9a9;">Scan a QR code to automatically fill both server address and and signup key</p>
	<br />
	<button
		...
		}}><QrCode class="align-middle" style="height: 16px; width: auto;" /> Scan QR Code</button
	>
	<br />
	<button class="button-primary width250px" style="width: 90%;" onclick={() => connectCheck()}>Connect</button>
</div>
